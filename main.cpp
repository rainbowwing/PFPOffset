...
int main(int argc, char *argv[]) {
    ...
    vector<chrono::time_point<chrono::system_clock> > time_path;
    ...
    for (int i = 0; i < mesh->FaceSize(); i++) {
        double minx = *set < double > {(mesh->fast_iGameVertex[mesh->fast_iGameFace[i].vh(0)] -
                                        mesh->fast_iGameVertex[mesh->fast_iGameFace[i].vh(1)]).norm(),
                                       (mesh->fast_iGameVertex[mesh->fast_iGameFace[i].vh(1)] -
                                        mesh->fast_iGameVertex[mesh->fast_iGameFace[i].vh(2)]).norm(),
                                       (mesh->fast_iGameVertex[mesh->fast_iGameFace[i].vh(2)] -
                                        mesh->fast_iGameVertex[mesh->fast_iGameFace[i].vh(0)]).norm()
        }.begin();
        double maxx = *set < double > {(mesh->fast_iGameVertex[mesh->fast_iGameFace[i].vh(0)] -
                                        mesh->fast_iGameVertex[mesh->fast_iGameFace[i].vh(1)]).norm(),
                                       (mesh->fast_iGameVertex[mesh->fast_iGameFace[i].vh(1)] -
                                        mesh->fast_iGameVertex[mesh->fast_iGameFace[i].vh(2)]).norm(),
                                       (mesh->fast_iGameVertex[mesh->fast_iGameFace[i].vh(2)] -
    ...
    } // do the step 1 , calculating the each vertex position after offsetting.
    time_path.push_back(std::chrono::system_clock::now());
    ...
    for (int i = 0; i < thread_num; i++)
        build_thread_pool[i]->join(); // do the step 2 , calculate spatial coverage from 6 tetrahedrons to a octahedron
                    ...
                    if ((faces_approximate_field[i].bound_face_id[j][0] >= 3 ||
                        ...
                        for (auto other: neighbor_face) { //Point_3, or Segment_3, or Triangle_3, or std::vector < Point_3 >
                            ...
                            if (res_tt) {
                                ...
                                } else if (std::vector<K2::Point_3> *vs = boost::get<std::vector<K2::Point_3 >>(
                                        &*res_tt)) {
    ...
    for (int i = 0; i < thread_num; i++)
        one_ring_select_thread_pool[i]->join();// do the step 2 , delete some useless face by one-ring neighbor
...
//    cout << tmp.x()<<" "<< tmp.y()<<" "<< tmp.z() << endl;
        ...
        bfs_thread_pool[i] = make_shared<std::thread>([&](int now_id) {
            for (int face_id = 0; face_id < fsize; face_id++) {
                ...
                while (!q.empty()) {
                    ...
                    for (auto j: neighbor) {
                        if (!is_visit.count(j)) {
                            double dist = cgal_vertex_triangle_dist(fh.second,
                                                                    (getGridVertex(j, 0) + getGridVertex(j, 7)) / 2,
                                                                    mesh);
                            double move_limit = *set < double >
                                                {(field_move_vertex[fh.second.vh(1)] -
                                                  mesh->fast_iGameVertex[fh.second.vh(0)]).norm(),
                                                 (field_move_vertex[fh.second.vh(2)] -
                                                  mesh->fast_iGameVertex[fh.second.vh(1)]).norm(),
                                                 (field_move_vertex[fh.second.vh(2)] -
                                                  mesh->fast_iGameVertex[fh.second.vh(0)]).norm()
                                                }.rbegin();
    ...
    for (int i = 0; i < thread_num; i++)
        bfs_thread_pool[i]->join();// do the step 3 , bfs to build grid
        ...
        each_grid_thread_pool[i] = make_shared<std::thread>([&](int now_id) {
            ...
            for (auto each_grid = frame_grid_mp.begin(); each_grid != frame_grid_mp.end(); each_grid++) {
                ...
                if (each_grid_cnt % (thread_num * 1000) == now_id) {
                    cout << "progress bar of step4 preparation  " << each_grid_cnt << "/" << (int) frame_grid_mp.size()
                         << endl;
                }
...
//                                                           getGridVertex(each_grid->first, 0));
    ...
    for (auto each_container_face: container_grid_face) {
        each_grid_face_list.push_back(
                {(size_t) each_container_face[0], (size_t) each_container_face[1], (size_t) each_container_face[2]});
        each_grid_face_list.push_back(
    ...
    for (unordered_map<grid, GridVertex, grid_hash, grid_equal>::iterator i = frame_grid_mp.begin();
            ...
            while (1) {
                ...
                if (!q.empty()) {
                    ...
                    if ((frame_grid_mp.size() - q.size()) % (frame_grid_mp.size() / 50) == 0) { //2520
                        cout << "progress bar of step4 & step 5 : " << (int) frame_grid_mp.size() - q.size() << "/"
                             << (int) frame_grid_mp.size() << endl;
                    ...
                    std::mutex mu2;
                    std::function<void(K2::Point_3, K2::Point_3, vector<int>, int)> dfs = [&](K2::Point_3 small,
                                                                                              K2::Point_3 big,
                                                                                              vector<int> face_list,
                                                                                              int depth) {
                        ...
                        PMP::polygon_soup_to_polygon_mesh(ps, each_grid_face_list, frame_poly,
                        ...
                        for (int i: face_list) {
                            ...
                            if (!useful)
                                useful = CGAL::Polygon_mesh_processing::do_intersect(*faces_approximate_field[i].poly,
                                                                                     frame_poly);
                            if (useful)
                                field_through_list.push_back(i);
                        }
                        if (state == ((1 << 8) - 1)) {
                            cout << "skip" << endl; // use skip mode
                            return;
                        }
                        function<bool(K2::Triangle_3)> triangle_through_grid = [&](K2::Triangle_3 tri) {
                            ...
                            PMP::polygon_soup_to_polygon_mesh(vs, face_type_012, this_face,
                                                              CGAL::parameters::all_default());
                        ...
                        for (int i = 0; i < field_through_list.size(); i++) {
                            for (int j = 0;
                                 j < faces_approximate_field[field_through_list[i]].bound_face_id.size(); j++) {
                                vector<MeshKernel::iGameVertex> tmp{
                                ...
                                if (faces_approximate_field[field_through_list[i]].bound_face_id[j][0] >= 3 ||
                                    faces_approximate_field[field_through_list[i]].bound_face_id[j][1] >= 3 ||
                                    faces_approximate_field[field_through_list[i]].bound_face_id[j][2] >= 3) { // 去掉原表面
                                    if (faces_approximate_field[field_through_list[i]].bound_face_useful[j] &&
                                        triangle_through_grid(tri_this)) {
                                        field_triangles.push_back(tri_this);
                                        field_triangles_part_aabbtree.push_back(tri_this);

                                        face_belong_field_source_id[tri_this.id()] = vector<int>{
                                                faces_approximate_field[field_through_list[i]].bound_face_id[j][0],
                                ...
                                if (const K2::Point_3 *p = boost::get<K2::Point_3>(&(item.first))) {
                                    ...
                                    if (intersection_v[this_field_belong_id].count(*p)) {
                                        if (!is_special.count(this_field_belong_id)) {
                                            if (faces_approximate_field[field_through_list[this_field_belong_id]].in_or_on_field(
                                                    this_center)) {
                                                flag = true;
                                                //cout <<"f2" << endl;
                                                break;
                        ...
                        for (auto this_face: field_triangles) {
                            ...
                            for (auto i: intersections) {
                                ...
                                if (this_field_belong_id !=
                                    this_face_belong_id /*&& !cutting_field_id[this_field_belong_id]*/) {
                                    ...
                                    if (const K2::Point_3 *p = boost::get<K2::Point_3>(&(i.first))) {
                                        ...
                                        for (K2::Segment_3 edge: {
                                                K2::Segment_3(this_face.vertex(0), this_face.vertex(1)),
                                        ...
                                        for (int l = 0; l < 3; l++) {
                                            for (int m = 0; m < 3; m++) {
                                                if (CGAL::squared_distance(this_face.vertex(l), t->vertex(m)) ==
                                                    CGAL::Epeck::FT(0)) {
                                                    cnt++;
                                                    break;
                                                }
                            ...
                            for (int i = 0; i < cutting_field_id.size(); i++) {
                                if (!cutting_field_id[i] && i != this_face_belong_id) {
                                    if (faces_approximate_field[field_through_list[i]].in_field(
                                            CGAL::centroid(this_face))) {
                        ...
                        std::function<vector<K2::Point_3>(K2::Triangle_3)> cutting_triangle_by_grid = [&](
                                K2::Triangle_3 this_face) {
                            ...
                            for (auto i: intersections) {
                                ...
                                } else if (const std::vector<K2::Point_3> *v = boost::get<std::vector<K2::Point_3>>(
                                        &(i.first))) {
                                    for (const auto &ii: *v)
                                        se.insert(ii);
                                }
                        ...
                        // 下面这段是和边框切割的代码
                        for (int maybe_used_face_id = 0;
                             maybe_used_face_id < maybe_used_face.size(); maybe_used_face_id++) {

                            vector<K2::Point_3> triangle_vertex_list = cutting_triangle_by_grid(
                                    maybe_used_face[maybe_used_face_id]);
                        ...
                        for (int i = 0; i < maybe_used_face.size(); i++) { // 处理单片面的切割
                            ...
                            for (auto tri: now_tri_list) {
                                ...
                                for (int j = 0; j < field_through_list.size(); j++) {
                                    if (positive_side.count(j) && intersection_v[j].size() % 2 == 0 &&
                                        tri.supporting_plane().has_on_positive_side(
                                                faces_approximate_field[field_through_list[j]].center)) {
                                        flag_positive = true;
                                    }
                                }
                                if (!flag && flag_positive) {
                                    ...
                                    for (int j = 0; j < field_through_list.size(); j++) {
                                        if (negative_side.count(j) && intersection_v_r[j].size() % 2 == 0 &&
                                            tri.supporting_plane().has_on_negative_side(
                                                    faces_approximate_field[field_through_list[j]].center)) {
                                            flag_negative = true;
                                        }
                                    }
                                ...
                                generated_face_list.push_back(
                                        K2::Triangle_3(tri.vertex(0), tri.vertex(2), tri.vertex(1)));
                    ...
                    dfs(small, big, face_list, 0); //use dfs build octree
    ...
    for (auto each_grid = frame_grid_mp.begin(); each_grid != frame_grid_mp.end(); each_grid++) {
        for (int i = 0; i < each_grid->second.generate_face_list.size(); i++) {// 局部id 全局id 按坐标轴排序 结果预存，每一个
            ...
            final_gen_face.push_back(
                    {final_gen_vertex.size(), final_gen_vertex.size() + 1, final_gen_vertex.size() + 2});
        ...
        for (auto each_grid = frame_grid_mp.begin(); each_grid != frame_grid_mp.end(); each_grid++) {
            for (int i = 0; i < each_grid->second.generate_face_list.size(); i++) {
                fprintf(file12, "v %lf %lf %lf\n",
                        CGAL::to_double(each_grid->second.generate_face_list[i].vertex(0).x()),
                        CGAL::to_double(each_grid->second.generate_face_list[i].vertex(0).y()),
                        CGAL::to_double(each_grid->second.generate_face_list[i].vertex(0).z()));
                fprintf(file12, "v %lf %lf %lf\n",
                        CGAL::to_double(each_grid->second.generate_face_list[i].vertex(1).x()),
                        CGAL::to_double(each_grid->second.generate_face_list[i].vertex(1).y()),
                        CGAL::to_double(each_grid->second.generate_face_list[i].vertex(1).z()));
                fprintf(file12, "v %lf %lf %lf\n",
                        CGAL::to_double(each_grid->second.generate_face_list[i].vertex(2).x()),
                        CGAL::to_double(each_grid->second.generate_face_list[i].vertex(2).y()),
                        CGAL::to_double(each_grid->second.generate_face_list[i].vertex(2).z()));
