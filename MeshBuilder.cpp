//
// Created by rainbowwing on 2023/2/25.
//
...
MeshBuilder::MeshBuilder(const vector<K2::Triangle_3> &generate_face_final, K2::Point_3 grid_min, K2::Point_3 grid_max,
                         vector<K2::Point_3> ext_v, double merge_eps) {
    this->merge_eps = merge_eps;
...
//        });
    ...
    for (int i = 0; i < pre_vsize; i += 3) {
        ...
        double r0 = sqrt(
                CGAL::to_double(K2::Segment_3(v[dsu.find_root(i)], v[dsu.find_root(i + 1)]).squared_length()) / 2);
        ...
        double r1 = sqrt(
                CGAL::to_double(K2::Segment_3(v[dsu.find_root(i + 1)], v[dsu.find_root(i + 2)]).squared_length()) / 2);
        ...
        double r2 = sqrt(
                CGAL::to_double(K2::Segment_3(v[dsu.find_root(i + 2)], v[dsu.find_root(i)]).squared_length()) / 2);
        Fuzzy_circle fc2(mid2, r2);
        std::list<K2::Point_3> result2;
        kdtree.search(std::back_inserter(result2), fc2);

        for (auto j: result0) {
            if (sqrt(CGAL::to_double(
                    CGAL::squared_distance(j, K2::Segment_3(v[dsu.find_root(i)], v[dsu.find_root(i + 1)])))) <=
                merge_eps) {
                if (ex_id.count(j.id())) {
                    if (ex_id[j.id()] == -1) {
                        int new_id = v.size();
                ...
                if (dsu.find_root(vmp[j.id()]) == dsu.find_root(i + 2)) {
                    goto end;
                } else if (dsu.find_root(vmp[j.id()]) == dsu.find_root(i) ||
                           dsu.find_root(vmp[j.id()]) == dsu.find_root(i + 1))
                    continue;
                else {
        ...
        for (auto j: result1) {
            if (sqrt(CGAL::to_double(
                    CGAL::squared_distance(j, K2::Segment_3(v[dsu.find_root(i + 1)], v[dsu.find_root(i + 2)])))) <=
                merge_eps) {
                if (ex_id.count(j.id())) {
                    if (ex_id[j.id()] == -1) {
                        int new_id = v.size();
                ...
                if (dsu.find_root(vmp[j.id()]) == dsu.find_root(i)) {
                    goto end;
                } else if (dsu.find_root(vmp[j.id()]) == dsu.find_root(i + 1) ||
                           dsu.find_root(vmp[j.id()]) == dsu.find_root(i + 2))
                    continue;
                else {
        ...
        for (auto j: result2) {
            if (sqrt(CGAL::to_double(
                    CGAL::squared_distance(j, K2::Segment_3(v[dsu.find_root(i + 2)], v[dsu.find_root(i)])))) <=
                merge_eps) {
                if (ex_id.count(j.id())) {
                    if (ex_id[j.id()] == -1) {
                        int new_id = v.size();
                ...
                if (dsu.find_root(vmp[j.id()]) == dsu.find_root(i + 1)) {
                    goto end;
                } else if (dsu.find_root(vmp[j.id()]) == dsu.find_root(i + 2) ||
                           dsu.find_root(vmp[j.id()]) == dsu.find_root(i))
                    continue;
                else {
        ...
        if (cut0.size() + cut1.size() + cut2.size() != 0) {
            int new_id = v.size();
            v.push_back(CGAL::centroid(
                    K2::Triangle_3(v[dsu.find_root(i)], v[dsu.find_root(i + 1)], v[dsu.find_root(i + 2)])));
            dsu.append();
            ...
            sort(cut0.begin(), cut0.end(), [&](int a, int b) {
                return CGAL::squared_distance(v[dsu.find_root(i)], v[a]) <
                       CGAL::squared_distance(v[dsu.find_root(i)], v[b]);
            });

            sort(cut1.begin(), cut1.end());
            cut1.resize(unique(cut1.begin(), cut1.end()) - cut1.begin());
            sort(cut1.begin(), cut1.end(), [&](int a, int b) {
                return CGAL::squared_distance(v[dsu.find_root(i + 1)], v[a]) <
                       CGAL::squared_distance(v[dsu.find_root(i + 1)], v[b]);
            });

            sort(cut2.begin(), cut2.end());
            cut2.resize(unique(cut2.begin(), cut2.end()) - cut2.begin());
            sort(cut2.begin(), cut2.end(), [&](int a, int b) {
                return CGAL::squared_distance(v[dsu.find_root(i + 2)], v[a]) <
                       CGAL::squared_distance(v[dsu.find_root(i + 2)], v[b]);
            });
